  ...
  return {
    ...
    getMissingEpisodes: async function () {
      const { rows } = await client.query(
        `SELECT full_name, removed, episode_number 
        FROM all_eps  
        LEFT JOIN (
          SELECT id, MAX(removed) as removed
          from date_removed 
          group by id
        ) as t2
        ON all_eps.id = t2.id 
    ...
    updateEpisodeName: async function (value, id) {
      await client.query("UPDATE all_eps SET full_name=($1) WHERE id=($2)", [
        value,
        id,
      ]);
    },
    ...
    setSpotifyStatus: async function ({ id }, bool) {
      await client.query(`UPDATE all_eps SET on_spotify=($1) WHERE id=($2)`, [
        bool,
        id,
      ]);
    },
